...
extension AudioEngineManager {
    ...
    private func scheduleNextBuffer(with playingSound: Playable? = nil, immediate: Bool = true) {
        ...
        // 랜덤화된 간격 계산
        let randomizedInterval = audioVariation.randomizedInterval
        ...
        timerSubscription = Timer.publish(
            ...
            .sink { [weak self] _ in
                ...
                // 랜덤화된 볼륨 적용
                let randomizedVolume = self.audioVariation.randomizedVolume
    ...
    private func playBufferImmediately(buffer: AVAudioPCMBuffer) {
        // 랜덤화된 볼륨/피치 적용
        let randomizedVolume = audioVariation.randomizedVolume

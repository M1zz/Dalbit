    ...
    private var topBar: some View {
        HStack(spacing: DS.Spacing.sm) {
            ...
            GlassSegmentedControl(
                items: Segment.allCases.map { ($0, $0.title) },
                selection: $segment
            )
            .frame(maxWidth: 240)
        ...
        // ⚠️ 바 전체에 재질을 깔지 않는다 — 화면 폭을 가로지르는 반투명 띠가 생기면서
        //    흐르는 별을 가로막아, 홈에서 이어지는 배경이 여기서 끊긴 것처럼 보인다.
        //    유리는 세그먼트 컨트롤 자기 자신만 두르고, 나머지는 배경이 그대로 비친다.
    }
}

...
enum UsageReportingService {
    ...
    static func currentMetrics() -> [String: Double] {
        ...
        // ── 사용 시간: 최근 창(window)으로 따로 본다.
        // ⚠️ 누적값(listenMin)만으로는 **지금** 쓰이는지 알 수 없다 — 한 달 전에 몰아 듣고
        //    떠난 사람과 매일 조금씩 듣는 사람이 같은 숫자로 보인다.
        metrics["listenMin7"] = tracker.recentMinutes(days: 7).rounded()
        metrics["listenMin30"] = tracker.recentMinutes(days: 30).rounded()

        // ── 사용 빈도: "며칠에 한 번 트는가". 리텐션과 달리 앱을 연 게 아니라 **들은** 날이다.
        metrics["activeDays7"] = Double(tracker.recentActiveDays(days: 7))
        metrics["activeDays30"] = Double(tracker.recentActiveDays(days: 30))
        metrics["sessions7"] = Double(tracker.recentSessions(days: 7))
        metrics["sessions30"] = Double(tracker.recentSessions(days: 30))
        metrics["bestStreak"] = Double(tracker.bestStreak)
        // 마지막 청취로부터 지난 일수 — 이탈을 가장 먼저 보여 준다. 한 번도 안 들었으면 보내지 않는다.
        if let idle = tracker.daysSinceLastListen {
            metrics["daysIdle"] = Double(idle)
        }

        // 시간대별 시작 분포(4시간 6칸) — 자기 전에 트는지, 새벽에 깨서 트는지가 갈린다.
        for (index, count) in tracker.hourBuckets.enumerated() {
            metrics["tod\(index)"] = Double(count)
        }
        ...
        // 인트로 무료 기간(첫 주)은 isTrial 로 따로 나가고, isPaid 에서는 빠진다 —
        // 아직 돈이 나가지 않은 사람이라 결제에 섞이면 전환율이 부푼다
        // (`SubscriptionManager.currentlyInIntroOffer`).
        metrics["flag.isPaid"] = UserDefaults.standard.bool(forKey: SubscriptionManager.didPayCacheKey) ? 1 : 0
        metrics["flag.isTrial"] = UserDefaults.standard.bool(forKey: SubscriptionManager.trialCacheKey) ? 1 : 0

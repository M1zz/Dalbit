...
struct UsageStatsView: View {
    ...
    private func diagnosticsSection() -> some View {
        let d = diagnostics
        card(L.Stats.deviceTitle.localized, note: L.Stats.deviceNote.localized) {
            ...
            footnote(String(format: L.Stats.installIDFormat.localized, d.installID))
        }
    }
    ...
    private func evidenceSection() -> some View {
        let e = evidence
        card(L.Stats.evidenceTitle.localized, note: L.Stats.evidenceNote.localized) {
            ...
            footnote(L.Stats.evidenceFootnote.localized)
        }
    }
    ...
    private func sessionLengthSection() -> some View {
        ...
        card(L.Stats.lengthTitle.localized, note: L.Stats.lengthNote.localized) {
            if total == 0 {
                emptyRow(L.Stats.lengthEmpty.localized)
            } else {
    ...
    private func trendSection() -> some View {
        let points = UsageReportingService.trend(unit: bucketUnit,
                                                 events: eventSamples,
                                                 snapshots: snapshots)
        card(L.Stats.trendTitle.localized, note: L.Stats.trendNote.localized) {
            ...
            if points.isEmpty {
                emptyRow(L.Stats.trendEmpty.localized)
            } else {
                Chart {
    ...
    private func retentionSection() -> some View {
        ...
        card(L.Stats.retentionTitle.localized, note: L.Stats.retentionNote.localized) {
            if rows.isEmpty {
                emptyRow(L.Stats.retentionEmpty.localized)
            } else {
    ...
    private func mixSection() -> some View {
        ...
        let total = buckets.reduce(0) { $0 + $1.count }
        card(L.Stats.mixTitle.localized, note: L.Stats.mixNote.localized) {
            if total == 0 {
                emptyRow(L.Stats.snapshotEmpty.localized)
            } else {
                Chart(buckets) { bucket in
                ...
                .chartXAxis { AxisMarks { AxisValueLabel().font(DS.Font.caption()) } }
                .frame(height: 160)
                footnote(L.Stats.mixFootnote.localized)
            }
        }
    }
    ...
    private func signalSection() -> some View {
        ...
        card(L.Stats.signalTitle.localized, note: nil) {
            if signals.isEmpty {
                emptyRow(L.Stats.snapshotEmpty.localized)
            } else {
    ...
    private func funnelSection() -> some View {
        ...
        card(L.Stats.funnelTitle.localized, note: L.Stats.funnelNote.localized) {
    ...
    private func eventSection() -> some View {
        card(L.Stats.eventTitle.localized, note: L.Stats.eventNote.localized) {
            if events.isEmpty {
                emptyRow(L.Stats.eventEmpty.localized)
            } else {
    ...
    private func crashSection() -> some View {
        card(L.Stability.title.localized, note: nil) {
    ...
    private func feedbackSection() -> some View {
        ...
        card(L.Stats.feedbackTitle.localized, note: nil) {

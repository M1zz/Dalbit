...
## 달 표면 질감 개선 + 부유감 강화 - 완료 (2026-08-20)

- [x] 표면을 fBm 노이즈 → **진짜 분화구**로 교체 (PlanetTextureFactory.moonMaps)
  - 노이즈만 쓰면 구름 얼룩처럼 보인다. 달이 달처럼 보이는 건 원형 분화구(파인 사발 + 솟은 턱),
    바다(마리아, 매끄럽고 어두운 평원), 큰 분화구에서 뻗는 광조(ray) 때문 — 셋을 직접 그린다
  - 분화구 110개를 구면에 균등 분포(z를 균등하게 뽑아야 극에 안 몰린다). 큰 것부터 그려
    작은 게 위에 얹히게 — 실제 생성 순서와 같은 인상
  - 바다 위에는 분화구를 덜 남긴다(용암이 덮었으므로)
  - 노멀맵 강도 4.5 → 9.0, 극 근처 가로 텍셀 밀집 보정(sin(lat))
  - 텍스처 768×384
- [x] 노출 조정: 처음엔 밝은 쪽이 하얗게 타서 디테일이 날아갔다 → 조명 6200 → 3900,
  고지대/바다 알베도 대비 완화(0.80/0.30 → 0.78/0.38)
- [x] 아주 느린 자전 추가 — 한 바퀴 약 9분. 멈춰 있지 않다는 게 느껴진다
  - 제스처 회전과 섞이지 않게 부모(rig=제스처) / 자식(구체=자전)으로 분리
  - 등록은 씬 생성 전 정적 토큰으로 (프로토타입에서 겪은 사고 재발 방지)
- [x] 부유 폭 확대 40/24 → 56/34pt — 달이 가운데 있지 않은 건 내가 우주를 떠다니기 때문

## "내가 부유하는" 느낌 — 시차(parallax) 도입 - 완료 (2026-08-20)

문제: 달만 혼자 떠다니면 "달이 움직인다"로 읽힌다. "내가 움직인다"로 읽히려면
모든 레이어가 **같은 방향으로, 거리에 반비례하는 크기로** 함께 움직여야 한다.

- [x] 흔들림 원본을 하나로 통일: swayX/swayY (-0.5~0.5). 모든 레이어가 이 하나를 공유
- [x] 시차 계수 — 별 0.10 < 앰비언트 천체 0.34 < 달 1.0
      (먼 것일수록 조금만 따라 움직인다)
- [x] 아주 느린 기울어짐 swayRoll — 우주에서는 몸이 조금씩 돌아간다.
      별 0.35배 / 앰비언트 0.7배 / 달 1배로 역시 차등 적용
- [x] 3D 달은 평면 이동에 더해 **카메라를 옆으로 옮긴다**(±0.16 씬 단위).
      달을 계속 바라보므로 화면상 위치는 그대로인데 보이는 각도만 바뀐다 = 진짜 시차.
      평면 이동만으로는 아무리 움직여도 '달이 미끄러진다'로 읽힌다
- [x] 시뮬레이터 렌더 확인 (회전으로 인한 모서리 결손 없음 — 별·앰비언트 레이어가 투명 Canvas라 무해)
- 조절 지점: floatRangeX/Y(폭), parallaxStars/parallaxCosmic(계수), swayRange(카메라 이동)

## 달 렌더링 선명도 개선 + 회전 방향 수정 - 완료 (2026-08-20)

- [x] 원인: 텍스처 768 → 앞면에 384텍셀뿐인데 화면은 660px(220pt @3x) → 언더샘플링이라 뿌옇다
  - 1536으로 상향 (앞면 768텍셀, 1.16배 오버샘플)
- [x] 고해상도 생성이 느려서 회색 구가 오래 보이지 않게 **프로그레시브**:
  384를 먼저 입히고 1536이 구워지면 바꿔 낀다
- [x] 해상도를 올리려면 분화구 검사 최적화가 선행돼야 했다 — 텍셀마다 110개를 전부 검사하면
  1536에서 1억 회가 넘는다 → **위도 밴드 96개로 버킷팅**해서 닿을 수 있는 것만 검사
- [x] 이방성 필터링(maxAnisotropy 16) — 구의 림은 텍스처를 비스듬히 훑어서
  이게 없으면 가장자리 분화구가 뭉개진다. 선명도 차이가 가장 큰 지점
- [x] 미세 요철(fbm 46.0) 추가 — 고해상도에서 표면이 매끈해 보이지 않게
- [x] 광조(ray) 완화 — 줄무늬가 또렷해서 긁힌 자국처럼 보였다 (세기 0.20 → 0.075, 개수 11 → 6)
- [x] **회전 방향 수정**: 3D로 옮기며 가로 회전에 -roll 을 넣어 손가락 반대로 굴렀다.
  RealityKit 은 +Y 오른손 법칙이라 양수일 때 앞면이 오른쪽으로 간다.
  2D 시절 markX = sin(roll) 과 같아지려면 +roll 이 맞다
- 참고: RealityViewEnvironment 에는 antialiasing 멤버가 없다(그 이름은 다른 타입에 있음)

## 천체가 지나가는 여행 — "달이 멀어지고 새 행성이 나온다" - 완료 (2026-08-20)

방향: 좌우로 흔들리는 것만으로는 "흔들린다"지 "떠다닌다"가 아니다. 실제로 지나쳐 가야 한다.

- [x] MoonJourney(신규): 멀리서 다가와 스쳐 지나가고 멀어지는 궤적을 3D 씬과 SwiftUI 가 공유
  - 슬롯 두 개가 반 주기씩 어긋나 하나가 멀어질 때 다른 하나가 다가온다 → 화면이 비지 않는다
  - x는 멀수록 더 벌어지고(원근이 겹쳐 '스쳐 지나감'이 분명해진다), z는 -farZ·u²
  - 크기는 스케일이 아니라 **원근**이 만든다 — 실제로 멀어지므로
  - 주기 120초, 얼굴 12개 순환
- [x] Moon3DView 를 두 천체 구조로 재작성: rig(궤적) → tilt(제스처 회전) → sphere(자전·표면)
  - 프레임을 2.1배로 넓히고 카메라 거리도 같은 배율로 → 화면상 크기는 그대로인데 지나갈 공간이 생긴다
- [x] 천체마다 다른 얼굴: moonMaps 에 seed(분화구 배치)·warmth(색기) 파라미터 추가
  - 멀리 있을 때 바꿔 끼워서 바뀌는 순간이 보이지 않게 (4초마다 확인)
- [x] 재생 심볼은 MoonJourney.centrality 로 연동 — 중앙이 비었을 땐 같이 사라진다
      (허공에 재생 표시만 떠 있으면 이상하다)

### 겪은 버그
- [x] **크래시**: bodyIndex 가 절대 시각 기반이라 6천만이 넘었고, 그 값이 노이즈 좌표(seed)로
  들어가 Int32 변환에서 트랩. → 작은 값(0~11)으로 순환시키고, valueNoise 에도 안전장치 추가
- [x] 분화구 테두리에 또렷한 원 자국: 알베도를 계단식(t < 0.9 ? -0.05 : 0.07)으로 바꾼 탓.
  smoothstep 으로 부드럽게 → 그려 넣은 것처럼 보이던 링이 사라짐
